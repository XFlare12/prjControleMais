...
namespace prjIntegradoBD
{
    public partial class AdminForm : Form
    {
        ...
        private void btnMostrarEmpregado_Click(object sender, EventArgs e)
        {
        ...
        {
            DeletarRegistro("Funcionarios", "funcionário");
        }
        ...
        private void btnAlterarProduto_Click(object sender, EventArgs e)
        {
            ...
            {
                // Exibe mensagem informando que a tabela será atualizada
                MessageBox.Show("Atualizando a tabela de produtos.");

                // Atualiza a tabela de produtos no DataGridView
                MostrarTabela("Produto");
            };
        ...
        {
            string input = Microsoft.VisualBasic.Interaction.InputBox(
                "Qual o ID do funcionário que deseja alterar?",
                "Alterar Funcionário",
                "");
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT COUNT(*) FROM Funcionarios WHERE Id = @id";
                    SqlCommand command = new SqlCommand(query, connection);
                    command.Parameters.AddWithValue("@id", id);
                    ...
                    if (count > 0)
                    {
                        AlteraFuncionario alteraFuncionarioForm = new AlteraFuncionario(this, id);

                        // Assina o evento FuncionarioAlterado
                        alteraFuncionarioForm.FuncionarioAlterado += (s, ea) =>
                        {
                            // Atualiza a tabela após o funcionário ser alterado
                            MessageBox.Show("Atualizando tabela de funcionários.");
                            AtualizarTabelaFuncionarios();
                        };

                        alteraFuncionarioForm.Show();
                    }
                    else
                    {
                        MessageBox.Show($"Nenhum funcionário encontrado com o ID {id}.");
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Erro ao buscar o funcionário: {ex.Message}");
            }
        }

        // Método para atualizar a tabela de funcionários
        ...
        {
            // Cria uma instância do formulário AdicionarFuncionario
            AdicionarFuncionario adicionarFuncionarioForm = new AdicionarFuncionario();

            // Exibe o formulário de maneira modal
            adicionarFuncionarioForm.ShowDialog();

            // Após fechar o formulário de adicionar, atualiza a tabela de funcionarios
            MostrarTabela("Funcionarios");
        }
    }
    }